...
exports.displaymedicine = async (req, resp) => {
    try {
        ...
        if (medicine == "") {
            success = false
            return resp.status(400).send({ success, message: "No Medicine Available" });
        }
        const all_products = medicine.map(products => ({
            ...
            i_path: path.join(__dirname, '../uploads', products.medicine_image),
...
exports.updatemedicine = async (req, resp) => {
    try {
        ...
        if (req.file) {
            ...
            const i_path = path.join(__dirname, '../', 'uploads', medicine_search.medicine_image);
            // console.log(i_path);
            fs.unlink(i_path, (err) => {
                if (err) {
                    return resp.status(400).send({ message: err });
...
exports.deletemedicine = async (req, resp) => {
    try {
        ...
        const medicine_find = await medicineModel.findOne({ _id: med_id });
        ...
        const i_path = path.join(__dirname, '../', 'uploads', medicine_image_name);
        ...
        //     return resp.status(400).send({message:"medicine not found"});
        // }
        const medicine = await medicineModel.deleteOne({ _id: med_id });
        success = true
        resp.status(200).send({ success, message: `${mname} was deleted` });    // .
...
exports.hideUnhideProduct = async (req, resp) => {
    try {
        ...
        const medicine_id = req.params.id;
        const medicine_find = await medicineModel.findOne({ _id: medicine_id });
        // const mname = medicine_find.medicine_name;
        if (medicine_find == null) {
            return resp.status(400).send({ message: "Medicine not found" });
        }
        ...
        let result;
        if (medicine_find.hide_status == false) {
            result = await medicineModel.updateOne(
                {
                    _id: medicine_id
                },
                {
            ...
            const cart_find = await cartModel.find({ "all_cart_data.medicine_id": medicine_id });
            if (cart_find != null) {
                cart_find.map(async (cart) => {
                    cart.all_cart_data.map(async (product) => {
                        if (product.medicine_id == medicine_id) {
                            let new_total_amount = cart.total_amount - product.amount;
                            const result = await cartModel.updateOne(
                                {
                                    _id: cart._id,
                                    "all_cart_data.medicine_id": medicine_id
                                },
                                {
                                    $set: {
        ...
        else if (medicine_find.hide_status == true) {
            result = await medicineModel.updateOne(
                {
                    _id: medicine_id
                },
        ...
        if (result.modifiedCount != 0) {
            success = true;
            return resp.status(200).send({ success, message: `${medicine_find.medicine_name} was updated` });
        }
        resp.status(400).send({ success, message: `${medicine_find.medicine_name} was not updated` });
...
exports.searchmedicine = async (req, resp) => {
    try {
        ...
        const filter = { medicine_name: new RegExp(medicine_name, 'i') };
        const medicine = await medicineModel.find(filter);

        if (medicine == "") {
            // resp.status(400).send({message:"Users not found"});
            success = false
            resp.status(400).send({ success, message: "medicine not found" });
            return;
        }
...
exports.searchmedicineClient = async (req, resp) => {
    try {
        const medicine_name = req.params.mname;
        const filter = { medicine_name: new RegExp(medicine_name, 'i'), hide_status: false };
        const medicine = await medicineModel.find(filter);
...
exports.displaymedicineClient = async (req, resp) => {
    try {
        ...
        resp.status(200).send(all_products);
        // resp.status(200).send({success,all_products});
